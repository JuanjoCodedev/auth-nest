...
export class RolesService {
  ...
  async createRol(body: RolesDto) {
    const existingRole = await this.rolesRepository.findOne({ where: { rol: body.rol } });

    if (existingRole) throw new ConflictException(`El rol "${body.rol}" ya está en uso. Por favor, prueba con otro nombre.`);
