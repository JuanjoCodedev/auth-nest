...
export class PermissionsService {
  ...
  async newPermission(body: PermissionsDto) {
    const permission = await this.permissionsRepository.findOne({ where: { permissions: body.permissions } });

    if (permission) throw new ConflictException(`El permiso "${body.permissions}" ya está en uso. Por favor, prueba con otro nombre.`);
