...
export class MunicipalityService {
    ...
    public async searchMunicipality(page: number = 1, limit: number = 10, nameMunicipality?: string, departmentId?: number, isCity?: boolean) {
        ...
        const [municipalities, totalCount] = await this._municipalityRepository.findAndCount({
            where: filters,
            take: limit,
            skip: (page - 1) * limit,
        ...
        return { data: municipalities, totalCount, totalPages: Math.ceil(totalCount / limit), currentPage: page };
    }
